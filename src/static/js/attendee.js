class Attendee {
  constructor(name, path) {
    this.name = name;
    this.path = path;
  }
}

//add more attendees here
let galleryAttendees = [
  new Attendee("Bobby Brady", "../static/images/attendee-bobby-brady.jpg"),
  new Attendee("Carol Brady", "../static/images/attendee-carol-brady.jpg"),
  new Attendee("Cindy Brady", "../static/images/attendee-cindy-brady.jpg"),
  new Attendee("Greg Brady", "../static/images/attendee-greg-brady.jpg"),
  new Attendee("Jan Brady", "../static/images/attendee-jan-brady.jpg"),
  new Attendee("Marcia Brady", "../static/images/attendee-marcia-brady.jpg"),
  new Attendee("Mike Brady", "../static/images/attendee-mike-brady.jpg"),
  new Attendee("Peter Brady", "../static/images/attendee-peter-brady.jpg"),
  new Attendee("Clint Rutkas", "../static/images/attendee-clint.jpg"),
  new Attendee("Golnaz", "../static/images/attendee-golnaz.jpg"),
  new Attendee("Royal Party", "../static/images/attendee-royal-family.jpg"),
  new Attendee("Tracy Morgan", "../static/images/attendee-tracy-morgan.jpg"),
  new Attendee("Barack Obama", "../static/images/attendee-barack-obama.png"),
  new Attendee(
    "John Krasinski",
    "../static/images/attendee-john-krasinski.png"
  ),
  new Attendee("Sleeping Puppy", "../static/images/attendee-puppy.png"),
  new Attendee("Meeting Llama", "../static/images/attendee-llama.png"),
  new Attendee("Colin Jost", "../static/images/attendee-colin-jost.png"),
  new Attendee("Jason Fried", "../static/images/attendee-jason-fried.png"),
];
